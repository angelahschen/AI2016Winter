...
;
(defun helper-at-least-one-color (n c k)
	(cond
		((= c k) (cons (node2var n k k) nil))
		(t (cons (node2var n c k) (helper-at-least-one-color n (+ 1 c) k)) )
	)
)

(defun at-least-one-color (n c k)
	(cons (helper-at-least-one-color n c k) nil)
)
...
(defun helper-at-most-one-color (c start end)
	(cond
		((= start end) (cons(cons (- c) (cons (- start) nil))nil))
		(t (cons (cons (- c) (cons (- start) nil)) (helper-at-most-one-color c (+ start 1) end) ) )
	)
)
...
(defun at-most-one-color (n c k)
	(cond
		((= (node2var n c k) (node2var n k k)) nil)
		((= (node2var n c k) (- (node2var n k k) 1))  (cons (cons (+ 1 (- (node2var n k k)) )  (cons (- (node2var n k k)) nil) ) nil) )
		(t   (append  (at-most-one-color n (+ 1 c) k) (helper-at-most-one-color (node2var n c k) (+ 1 (node2var n c k)) (node2var n k k)) )  )	
	)
)
...
(defun generate-node-clauses (n k)
	(append (at-most-one-color n 1 k) (at-least-one-color n 1 k))
)
...
(defun helper-generate-edge-clauses (n1 n2 k)
	(cond
		((= k 0) (cons (cons (- n1) (cons(- n2)nil)) nil))
		(t  (cons (cons (-(+ n1 k)) (cons(-(+ n2 k))nil))  (helper-generate-edge-clauses n1 n2 (- k 1) ) ))
	)
)

(defun generate-edge-clauses (e k)
	(helper-generate-edge-clauses (node2var (first e) 1 k) (node2var (second e) 1 k) (- k 1)  )
)
